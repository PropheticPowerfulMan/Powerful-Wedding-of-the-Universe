      ...
      if (channel === 'email' || channel === 'both') {
        if (guestEmail) {
          ...
          if (!dryRun) {
            ...
            const emailHtml = buildInvitationEmailHtml(invitationLabel, emailText);
            const emailErr = await sendEmail(guestEmail, emailSubject, emailText, hostEmailRecipients, emailHtml);
            if (emailErr) {
